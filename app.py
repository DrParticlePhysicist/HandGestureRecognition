#!/usr/bin/env python
# -*- coding: utf-8 -*-
"""
Real-Time Hand Gesture Recognition
-----------------------------------
Main application file for capturing video from webcam, detecting hands using
MediaPipe, and classifying gestures using a TensorFlow Lite model.

Features:
- Inference mode (classify gestures with a trained model).
- Logging mode (collect new gesture data into CSV for training).
"""

import os
import csv
import copy
import argparse
import itertools
from collections import deque

import cv2 as cv
import numpy as np
import mediapipe as mp

from utils import CvFpsCalc
from model.keypoint_classifier.keypoint_classifier import KeyPointClassifier


# --------------------------------------------------------------------------
# Argument Parser
# --------------------------------------------------------------------------
def get_args():
    """Parse command-line arguments for device and capture resolution."""
    parser = argparse.ArgumentParser()
    parser.add_argument("--device", type=int, default=0,
                        help="Camera device index (default=0)")
    parser.add_argument("--width", type=int, default=960,
                        help="Capture width")
    parser.add_argument("--height", type=int, default=540,
                        help="Capture height")
    return parser.parse_args()


# --------------------------------------------------------------------------
# Main Application
# --------------------------------------------------------------------------
def main():
    """Main loop for real-time gesture recognition."""
    args = get_args()
    cap_device, cap_width, cap_height = args.device, args.width, args.height

    # Initialize camera
    cap = cv.VideoCapture(cap_device)
    cap.set(cv.CAP_PROP_FRAME_WIDTH, cap_width)
    cap.set(cv.CAP_PROP_FRAME_HEIGHT, cap_height)

    if not cap.isOpened():
        print("❌ ERROR: Could not access camera.")
        return

    # Initialize Mediapipe hands detector
    mp_hands = mp.solutions.hands
    hands = mp_hands.Hands(
        static_image_mode=False,
        max_num_hands=1,
        min_detection_confidence=0.7,
        min_tracking_confidence=0.5,
    )

    # Load gesture classifier model safely
    keypoint_classifier = None
    tflite_path = "model/keypoint_classifier/keypoint_classifier.tflite"
    if os.path.exists(tflite_path) and os.path.getsize(tflite_path) > 10:
        keypoint_classifier = KeyPointClassifier(model_path=tflite_path)
    else:
        print("⚠️ No trained model found. Inference disabled, only logging mode available.")

    # Load labels if available
    label_path = "model/keypoint_classifier/keypoint_classifier_label.csv"
    if os.path.exists(label_path):
        with open(label_path, encoding="utf-8-sig") as f:
            keypoint_classifier_labels = [row[0] for row in csv.reader(f)]
    else:
        keypoint_classifier_labels = []

    # Utilities
    cvFpsCalc = CvFpsCalc(buffer_len=10)

    # Histories
    point_history = deque(maxlen=16)

    # Modes
    mode = 0   # 0 → inference, 1 → logging
    number = -1

    # ------------------- Main Loop -------------------
    while True:
        fps = cvFpsCalc.get()
        key = cv.waitKey(10)

        # ESC → exit
        if key == 27:
            break

        # Mode selection
        number, mode = select_mode(key, mode)

        # Capture frame
        ret, image = cap.read()
        if not ret:
            print("❌ ERROR: Failed to capture image.")
            break

        # Mirror image for natural UX
        image = cv.flip(image, 1)
        debug_image = copy.deepcopy(image)

        # Hand Detection
        image = cv.cvtColor(image, cv.COLOR_BGR2RGB)
        image.flags.writeable = False
        results = hands.process(image)
        image.flags.writeable = True

        # If hand detected
        if results.multi_hand_landmarks:
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                                                  results.multi_handedness):
                # 1. Bounding box
                brect = calc_bounding_rect(debug_image, hand_landmarks)

                # 2. Landmark list
                landmark_list = calc_landmark_list(debug_image, hand_landmarks)

                # 3. Preprocess landmarks
                pre_processed_landmark_list = pre_process_landmark(
                    landmark_list
                )

                # 4. Logging (save to dataset)
                logging_csv(number, mode, pre_processed_landmark_list)

                # 5. Inference
                if mode == 0 and keypoint_classifier is not None:
                    hand_sign_id = keypoint_classifier(pre_processed_landmark_list)
                    hand_sign_text = keypoint_classifier_labels[hand_sign_id]
                elif mode == 1:
                    hand_sign_text = "Logging..."
                else:
                    hand_sign_text = "No model"

                # 6. Draw results
                debug_image = draw_bounding_rect(True, debug_image, brect)
                debug_image = draw_info_text(
                    debug_image, brect, handedness, hand_sign_text
                )

        # Draw general info
        debug_image = draw_info(debug_image, fps, mode, number)

        # Show window
        cv.imshow("Hand Gesture Recognition", debug_image)

    # Cleanup
    cap.release()
    cv.destroyAllWindows()


# --------------------------------------------------------------------------
# Helper Functions
# --------------------------------------------------------------------------
def select_mode(key, mode):
    """
    Switch between inference and logging modes.

    Args:
        key: pressed key value
        mode: current mode (0 or 1)

    Returns:
        number (digit pressed) and updated mode
    """
    number = -1
    if 48 <= key <= 57:  # keys 0–9
        number = key - 48
    if key == 107:  # 'k' → toggle logging/inference
        mode = 1 - mode
    return number, mode


def calc_bounding_rect(image, landmarks):
    """Calculate bounding box from hand landmarks."""
    image_width, image_height = image.shape[1], image.shape[0]
    landmark_array = np.array([
        [int(landmark.x * image_width), int(landmark.y * image_height)]
        for landmark in landmarks.landmark
    ])
    x, y, w, h = cv.boundingRect(landmark_array)
    return [x, y, x + w, y + h]


def calc_landmark_list(image, landmarks):
    """Convert Mediapipe landmarks into 2D pixel coordinates list."""
    image_width, image_height = image.shape[1], image.shape[0]
    return [
        [int(landmark.x * image_width), int(landmark.y * image_height)]
        for landmark in landmarks.landmark
    ]


def pre_process_landmark(landmark_list):
    """
    Normalize landmarks:
    - Make wrist (index 0) the origin.
    - Flatten to 1D vector.
    - Normalize values between -1 and 1.
    """
    temp_landmark_list = copy.deepcopy(landmark_list)

    # Wrist as origin
    base_x, base_y = temp_landmark_list[0]
    for point in temp_landmark_list:
        point[0] -= base_x
        point[1] -= base_y

    # Flatten
    temp_landmark_list = list(itertools.chain.from_iterable(temp_landmark_list))

    # Normalize
    max_value = max(list(map(abs, temp_landmark_list)))
    return [n / max_value for n in temp_landmark_list]


def logging_csv(number, mode, landmark_list):
    """
    Save landmark data to CSV when in logging mode.

    Args:
        number: digit key (gesture label)
        mode: current mode
        landmark_list: preprocessed landmark vector
    """
    if mode == 1 and (0 <= number <= 9):
        csv_path = "model/keypoint_classifier/keypoint.csv"
        with open(csv_path, "a", newline="") as f:
            writer = csv.writer(f)
            writer.writerow([number, *landmark_list])


def draw_bounding_rect(use_brect, image, brect):
    """Draw bounding rectangle around detected hand."""
    if use_brect:
        cv.rectangle(image, (brect[0], brect[1]), (brect[2], brect[3]),
                     (0, 0, 0), 1)
    return image


def draw_info_text(image, brect, handedness, hand_sign_text):
    """Overlay handedness and gesture label text on image."""
    cv.rectangle(image, (brect[0], brect[1]),
                 (brect[2], brect[1] - 22), (0, 0, 0), -1)
    info_text = handedness.classification[0].label
    if hand_sign_text != "":
        info_text += ":" + hand_sign_text
    cv.putText(image, info_text, (brect[0] + 5, brect[1] - 4),
               cv.FONT_HERSHEY_SIMPLEX, 0.6, (255, 255, 255), 1, cv.LINE_AA)
    return image


def draw_info(image, fps, mode, number):
    """Overlay FPS, mode, and label number on frame."""
    cv.putText(image, "FPS:" + str(fps), (10, 30),
               cv.FONT_HERSHEY_SIMPLEX, 1.0, (255, 255, 255), 2, cv.LINE_AA)
    if mode == 1:
        cv.putText(image, "MODE: Logging", (10, 60),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6,
                   (255, 255, 255), 1, cv.LINE_AA)
        if 0 <= number <= 9:
            cv.putText(image, "NUM:" + str(number), (10, 80),
                       cv.FONT_HERSHEY_SIMPLEX, 0.6,
                       (255, 255, 255), 1, cv.LINE_AA)
    else:
        cv.putText(image, "MODE: Inference", (10, 60),
                   cv.FONT_HERSHEY_SIMPLEX, 0.6,
                   (255, 255, 255), 1, cv.LINE_AA)
    return image


# --------------------------------------------------------------------------
# Entry Point
# --------------------------------------------------------------------------
if __name__ == "__main__":
    main()
